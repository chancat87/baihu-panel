...
### 最近更新
**2026.09.04** - **IDE 底部 Terminal 控制台、运行环境持久化、任务配置批量更新、ConPTY 重叠修复与安全升级 (v1.1.28)**：编辑器运行终端升级为 VSCode 沉浸式底部 Panel 控制台；运行环境配置自动持久化；任务与仓库列表新增批量修改配置功能；优化仓库同步 Git 跟踪过滤；解决 Windows ConPTY 视口重绘导致命令重叠 Bug；升级依赖彻底修复 9 项 Dependabot 漏洞。  
