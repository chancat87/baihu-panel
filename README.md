...
### 最近更新
**2026.09.04** - **Windows ConPTY 视口防抖与命令重叠修复 (v1.1.29)**：为网页终端尺寸调整引入 150ms 防抖并优化 initialCommand 发送时机，彻底解决 Windows ConPTY 在视口初始化与动画调整时重绘导致命令重叠的 Bug；清理重复按键绑定与调试代码。  
**2026.09.04** - **IDE 底部 Terminal 控制台、运行环境持久化、任务配置批量更新与 Git 过滤优化 (v1.1.28)**：编辑器运行终端升级为 VSCode 沉浸式底部 Panel 控制台；运行环境配置实现前端自动持久化，点击【运行】无感调用；任务与仓库列表新增弹窗式批量修改配置功能；优化仓库同步 Git 跟踪文件过滤逻辑；升级 kin-openapi 依赖以修复 Dependabot 警告。  
**2026.08.21** - **通知过滤、小屏展示优化、移动端平滑滚动与 CLI 文档增强 (v1.1.27)**：新增全局及任务通知通道过滤；优化窄屏下任务工具栏宽度分配防动作按钮溢出；为手机端引入像素级原生滚动与 2.5x 滑动加速提升流畅度；优化超长任务名称排版防挤压重叠；优化非文本二进制/图片文件查看与下载；补充完善 CLI 参数及头部注释格式规范；修复 TS 编译报错。  
**2026.08.17** - **通知内容统一与 Options SDK、日志 SSE 状态帧协议与 Bug 修复 (v1.1.26)**：统一通知为 `content` 与 `format` 字段并上线 Node.js/Python SDK Options 设计（#161）；实现任务日志流 Stream-as-State 协议，实时同步任务结束状态；修复历史日志全屏弹窗删除失效问题（#157）；优化调度器与日志锁并发安全；升级 Go 1.26 运行时。  
